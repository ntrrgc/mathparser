...
ast::Multiplication::Multiplication(std::unique_ptr<ast::Node> operand1, std::unique_ptr<ast::Node> operand2) {
    setOperand1(move(operand1));
    setOperand2(move(operand2));
}

double ast::Multiplication::value(const ast::Context& ctx) const {
    return operand1()->value(ctx) * operand2()->value(ctx);
}

string ast::Multiplication::toString() const {
    ostringstream ss;
